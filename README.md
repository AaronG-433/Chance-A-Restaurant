...
### How it works
Using the react library, I use an premade API key to make a call to the Google Maps API (specifically the Places API). This request goes through a 'cors-anywher' app running on Heroky before continuing on to go Google. Upon receiving the data, I parse it, randomly pick a restaurant, and display it for the user. From there, the user can choose to keep the displayed restaurant or randomly load a new one from the remaining choices.
